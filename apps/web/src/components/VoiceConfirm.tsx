...
export function VoiceConfirm({
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-[var(--c-bg)]/70 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm bg-[var(--c-panel)] border border-[var(--c-border)]/60 rounded-2xl p-6 space-y-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-3 py-2 bg-[var(--c-bg)]/60 border border-[var(--c-border)]/40 rounded-xl">
          <p className="text-[10px] uppercase tracking-widest text-[var(--c-muted2)] mb-1">{t('voice.heard')}</p>
          <p className="text-sm text-[var(--c-muted)] italic">"{transcript}"</p>
        </div>
        ...
        {unknown ? (
          <button type="button" onClick={onClose} className="w-full py-2.5 bg-[var(--c-border)]/60 rounded-xl text-sm font-medium text-white">
            {t('common.close')}
          </button>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            <button type="button" onClick={onEdit} className="py-2.5 bg-[var(--c-bg)] border border-[var(--c-border)]/60 rounded-xl text-sm font-medium text-white hover:bg-[var(--c-border)]/30">
              {t('voice.edit')}
            </button>
