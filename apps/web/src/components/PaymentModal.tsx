...
export default function PaymentModal({ saleId, unitName, currency, onClose, onUpdated }: Props) {
  ...
  const inp = 'bg-[var(--c-bg)] border border-[var(--c-border)]/50 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-[#FF6B1A]/50';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--c-bg)]/70 backdrop-blur-sm" onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} className="w-full max-w-2xl bg-[var(--c-panel)] border border-[var(--c-border)]/60 rounded-2xl p-6 space-y-5 max-h-[90vh] overflow-y-auto custom-scrollbar">
        <div className="flex items-center justify-between">
          ...
          <button onClick={onClose} aria-label="Yopish" className="w-11 h-11 -mr-2 inline-flex items-center justify-center rounded-lg text-[var(--c-muted)] hover:text-white hover:bg-white/5"><Icon icon="lucide:x" className="w-5 h-5" /></button>
        </div>

        {/* Yangi to'lov qo'shish */}
        <div className="grid grid-cols-2 md:grid-cols-3 gap-2.5 p-4 bg-[var(--c-bg)]/50 border border-[var(--c-border)]/40 rounded-xl">
          ...
          <button onClick={handleAdd} disabled={saving} className="col-span-2 md:col-span-1 bg-[#FF6B1A] hover:bg-[#e55a10] rounded-lg py-2 font-semibold text-white text-sm disabled:opacity-50 flex items-center justify-center gap-1.5">
            <Icon icon="lucide:plus" className="w-4 h-4" /> {saving ? 'Saqlanmoqda...' : "To'lov qo'shish"}
          </button>
        </div>

        {/* Tarix — vaqt bo'yicha o'sish tartibida */}
        <div className="border border-[var(--c-border)]/40 rounded-xl overflow-hidden">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[var(--c-bg)]/40 text-[10px] uppercase tracking-widest text-[var(--c-muted)]">
            ...
            <tbody className="divide-y divide-[var(--c-border)]/30">
              {loading ? (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-[var(--c-muted)]">Yuklanmoqda...</td></tr>
              ) : payments.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-[var(--c-muted)]">Hali to'lov kiritilmagan</td></tr>
              ) : (
                payments.map((p, i) => (
                  <tr key={p.id} className="hover:bg-white/5">
                    <td className="px-4 py-2.5 text-[var(--c-muted)]">{i + 1}. {fmtDate(p.paidAt)}</td>
                    <td className="px-4 py-2.5 font-semibold text-[#10B981]">{fmtMoney(Number(p.amount), p.currency)}</td>
                    <td className="px-4 py-2.5 text-[var(--c-muted)]">{p.location || '—'}</td>
                    <td className="px-4 py-2.5 text-[var(--c-muted)]">{p.method || '—'}</td>
                    <td className="px-4 py-2.5 text-[var(--c-muted)]">{p.note || '—'}</td>
        ...
        <div className="flex justify-end items-center gap-2 text-sm">
          <span className="text-[var(--c-muted)]">Jami to'langan:</span>
