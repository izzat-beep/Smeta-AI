    ...
    <div className="p-4 lg:p-10 max-w-[1280px] mx-auto w-full space-y-8">
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
        <div className="space-y-2">
          ...
          <p className="text-[var(--c-muted)]">{t('realtors.subtitle')}</p>
        </div>
        <button onClick={() => setEdit('new')} className="flex items-center gap-2 px-6 py-2.5 bg-[#FF6B1A] hover:bg-[#e55a10] text-white rounded-xl font-bold text-sm shadow-[0_0_20px_rgba(255,107,26,0.2)]">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-end gap-2">
          <span className="text-xs text-[var(--c-muted)]">{t('realtors.currency')}</span>
          <div className="flex bg-[var(--c-border)]/40 border border-[var(--c-border)]/40 rounded-xl p-1">
            {(['UZS', 'USD'] as Currency[]).map((c) => (
              <button key={c} onClick={() => setCur(c)} className={`px-4 py-1 text-xs font-bold rounded-lg ${cur === c ? 'bg-[var(--c-panel)] text-white shadow-sm' : 'text-[var(--c-muted)]'}`}>{c}</button>
            ))}
          </div>
        </div>
      ...
      <div>
        <h2 className="text-sm font-bold uppercase tracking-widest text-[var(--c-muted)] mb-3">{t('realtors.listTitle')}</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <div key={r.id} className="glass-panel rounded-2xl p-5 space-y-3">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {r.phone && <p className="text-[12px] text-[var(--c-muted)] flex items-center gap-1"><Icon icon="lucide:phone" className="w-3 h-3" />{r.phone}</p>}
                </div>
              ...
              <div className="flex items-center gap-4 text-sm">
                <span className="text-[var(--c-muted)]">{t('realtors.salesCount', { count: r.salesCount })}</span>
              </div>
              <div className="pt-2 border-t border-[var(--c-border)]/40 space-y-1">
                {Object.keys(r.totalsByCurrency).length === 0 ? (
                  <p className="text-[12px] text-[var(--c-muted2)]">{t('realtors.noCommission')}</p>
                ) : (
                  Object.entries(r.totalsByCurrency).map(([c, tot]) => (
                    <div key={c} className="flex justify-between text-[12px]">
                      <span className="text-[var(--c-muted)]">{t('realtors.commissionCur', { cur: c })}</span>
          ...
          {realtors.length === 0 && (
            <div className="col-span-full glass-panel rounded-2xl p-10 text-center text-[var(--c-muted)]">
      ...
      <div>
        <h2 className="text-sm font-bold uppercase tracking-widest text-[var(--c-muted)] mb-3">{t('realtors.salesTitle')}</h2>
              ...
              <thead>
                <tr className="bg-[var(--c-bg)]/30 border-b border-[var(--c-border)]/40">
                  {[t('realtors.colRealtor'), t('realtors.colUnit'), t('realtors.colBuyer'), t('realtors.colPrice'), t('realtors.colCommission'), t('realtors.colDate')].map((h) => (
                    <th key={h} className="px-5 py-4 text-[10px] font-bold uppercase tracking-widest text-[var(--c-muted)] whitespace-nowrap">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--c-border)]/30">
                {sales.map((x) => (
                  <tr key={x.id} className="hover:bg-white/5">
                    ...
                    <td className="px-5 py-4 text-sm text-[var(--c-muted)] whitespace-nowrap">{x.unitName}</td>
                    <td className="px-5 py-4 text-sm text-[var(--c-muted)] whitespace-nowrap">{x.buyerName}</td>
                    ...
                    <td className="px-5 py-4 text-sm text-[var(--c-muted)] whitespace-nowrap">{fmtDate(x.soldAt)}</td>
                  </tr>
                ))}
                {sales.length === 0 && (
                  <tr><td colSpan={6} className="px-5 py-12 text-center text-[var(--c-muted)]"><Icon icon="lucide:home" className="w-8 h-8 mx-auto mb-2 opacity-40" />{t('realtors.noSales')}</td></tr>
                )}
              </tbody>
            </table>
          </div>
...
function RealtorModal({ realtor, onClose, onDone }: { realtor: Realtor | null; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--c-bg)]/70 backdrop-blur-sm" onClick={onClose}>
      <form onClick={(e) => e.stopPropagation()} onSubmit={submit} className="w-full max-w-md bg-[var(--c-panel)] border border-[var(--c-border)]/60 rounded-2xl p-6 space-y-4">
        ...
        <div className="space-y-1.5">
          <label className="text-[12px] text-[var(--c-muted)]">{t('realtors.modal.name')}</label>
          <input value={name} onChange={(e) => setName(e.target.value)} required placeholder={t('realtors.modal.namePh')} className={inp} />
        </div>
        <div className="space-y-1.5">
          <label className="text-[12px] text-[var(--c-muted)]">{t('realtors.modal.phone')}</label>
