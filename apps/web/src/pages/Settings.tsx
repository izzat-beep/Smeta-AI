      ...
      <div className="space-y-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-[#BCC0C7] text-sm mt-1">{t('settings.subtitle')}</p>
          </div>
          ...
          <>
            ...
            <section className="glass-panel rounded-xl p-6 relative overflow-hidden">
              ...
              <div className="flex flex-col md:flex-row gap-6 items-start md:items-center mb-8">
                {avatarUrl ? (
                  <img src={avatarUrl} alt={fullName} className="w-20 h-20 rounded-full object-cover shadow-xl border-2 border-[#343841]" />
                ) : (
                ...
                <div className="space-y-1">
                  ...
                  <p className="text-[12px] text-[#BCC0C7] tracking-widest uppercase">{position || t('settings.noPosition')}</p>
                  <button type="button" className="mt-2 px-3 py-1.5 bg-[#16181D] border border-[#343841]/60 rounded-lg text-[12px] font-medium text-white hover:bg-[#1c1f26] transition-colors">
            ...
            <section className="glass-panel rounded-xl p-6">
              ...
              <div className="space-y-8">
                ...
                <div className="flex items-center justify-between p-4 bg-[#343841]/20 border border-[#343841]/30 rounded-2xl">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#BCC0C7]">{t('settings.baseCurrencyDesc')}</p>
                    </div>
                  </div>
                  <div className="flex bg-[#16181D]/80 border border-[#343841]/50 p-1 rounded-xl">
                    {(['UZS', 'USD'] as const).map((c) => (
                      <button
                        ...
                        className={currency === c ? 'px-4 py-1 bg-[#5555E7] text-white text-[12px] font-bold rounded-lg shadow-sm' : 'px-4 py-1 text-[#BCC0C7] text-[12px] font-medium'}
                      >
                        {c}
                      </button>
                    ))}
                ...
                <div className="p-4 bg-[#343841]/20 border border-[#343841]/30 rounded-2xl">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-[10px] text-[#BCC0C7]">{t('settings.exchangeRateDesc')}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-[#BCC0C7]">1 USD =</span>
                    <input
                      type="number"
                      value={usdRate}
                      onChange={(e) => setUsdRate(e.target.value)}
                      readOnly={!isOwner}
                      className={`w-40 bg-[#16181D]/50 border border-[#343841]/50 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-[#5555E7]/50 ${!isOwner ? 'opacity-60 cursor-not-allowed' : ''}`}
                    />
                    <span className="text-sm text-[#BCC0C7]">UZS</span>
                  </div>
                  {!isOwner && <p className="text-[10px] text-[#BCC0C7]/70 mt-2">{t('settings.exchangeRateOwnerOnly')}</p>}
                </div>

                {/* Birliklar */}
                <div>
                  <p className="text-[12px] font-semibold text-[#BCC0C7] tracking-widest uppercase mb-4">{t('settings.units')}</p>
                ...
                <div className="space-y-4">
                  ...
                  <ToggleItem icon="lucide:moon" label={t('settings.darkMode')} iconColor="text-[#818CF8]" checked={darkMode} onChange={setDarkMode} />
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex bg-[#16181D]/80 border border-[#343841]/50 p-1 rounded-xl">
                      <button type="button" onClick={() => changeLang('uz')} className={lang === 'uz' ? 'px-3 py-1 bg-[#5555E7] text-white text-[12px] font-bold rounded-lg' : 'px-3 py-1 text-[#BCC0C7] text-[12px] font-medium'}>{t('settings.langUz')}</button>
                      <button type="button" onClick={() => changeLang('ru')} className={lang === 'ru' ? 'px-3 py-1 bg-[#5555E7] text-white text-[12px] font-bold rounded-lg' : 'px-3 py-1 text-[#BCC0C7] text-[12px] font-medium'}>{t('settings.langRu')}</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-end gap-4 pt-4 pb-10">
              ...
              <button type="button" onClick={handleCancel} disabled={saving} className="px-6 py-2.5 text-[#BCC0C7] font-medium hover:text-white transition-colors disabled:opacity-50">
                {t('common.cancel')}
              </button>
  ...
  return (
    <div className="space-y-2">
      ...
      <input
        ...
        className={`w-full bg-[#16181D]/50 border border-[#343841]/50 rounded-xl px-4 py-2.5 text-sm text-white focus:outline-none focus:border-[#5555E7]/50 transition-all ${readOnly ? 'text-[#BCC0C7] cursor-not-allowed' : ''}`}
      />
    </div>
  );
}
  ...
  return (
    <button
      ...
      className={`px-3 py-2 rounded-xl text-[10px] font-medium text-center border transition-all cursor-pointer ${
        active ? 'bg-[#5555E7]/10 border-[#5555E7]/40 text-[#5555E7]' : 'bg-[#343841]/10 border-[#343841]/30 text-[#BCC0C7] hover:border-[#343841]/60'
      }`}
    >
      {label}
    </button>
