  ...
  return (
    <div className="p-4 lg:p-6 h-[calc(100vh-4rem)]">
      <div className="max-w-4xl mx-auto h-full flex flex-col glass-panel rounded-2xl border-[#343841]/40 overflow-hidden relative">
        ...
        {/* Header */}
        <div className="p-4 border-b border-[#343841]/40 bg-[#16181D]/20 flex items-center justify-between z-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] font-semibold text-[#BCC0C7] tracking-wider uppercase">Online • Aqlli Yordamchi</span>
              </div>
            </div>
          </div>
          <button
            onClick={() => { setMessages(messages.slice(0, 1)); setSessionId(null); }}
            className="p-2 text-[#BCC0C7] hover:text-white transition-colors"
            title="Yangi suhbat"
          >
        ...
        <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar z-10">
          <div className="flex justify-center">
            <span className="px-4 py-1 bg-[#343841]/30 rounded-full text-[10px] font-bold text-[#BCC0C7]">Bugun</span>
          </div>
          {messages.map((m, i) =>
            m.role === 'assistant' ? (
              ...
              <div key={i} className="space-y-1 flex flex-col items-end">
                <div className="max-w-[85%] p-4 bg-[#343841]/40 border border-[#343841]/50 rounded-tl-2xl rounded-br-2xl rounded-bl-2xl backdrop-blur-md">
        ...
        {/* Input */}
        <div className="p-4 border-t border-[#343841]/40 bg-[#16181D]/20 space-y-4 z-10">
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
            {SUGGESTIONS.map((s) => (
              <button
                ...
                onClick={() => send(s)}
                disabled={streaming}
                className="px-3 py-1.5 bg-[#343841]/30 border border-[#343841]/40 rounded-full text-[11px] text-[#BCC0C7] whitespace-nowrap hover:bg-[#343841]/50 transition-colors disabled:opacity-50"
              >
                {s}
              </button>
            ))}
          ...
          >
            <textarea
              ...
              className="w-full bg-[#16181D]/40 border border-[#343841]/60 rounded-2xl p-4 pr-14 text-sm text-white placeholder-[#BCC0C7]/60 focus:outline-none focus:border-[#3DF2FF]/50 transition-all resize-none min-h-[60px]"
            ...
            >
              <Icon icon={streaming ? 'lucide:loader' : 'lucide:send'} className={`w-5 h-5 text-[#16181D] ${streaming ? 'animate-spin' : ''}`} />
            </button>
          </form>
          <p className="text-[10px] text-[#BCC0C7]/60 italic text-center">AI xato qilishi mumkin, natijalarni tekshiring.</p>
        </div>
      </div>
    </div>
  );
